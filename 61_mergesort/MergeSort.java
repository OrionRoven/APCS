...
public class MergeSort
{
  ...
  private static int[] merge( int[] a, int[] b )
  {
    ...
    int bCtr = 0;
    int totalCtr = 0;
    while (a.length > aCtr && b.length > bCtr) {

      if (a[aCtr] < b[bCtr]) {
        returned[totalCtr] = a[aCtr];
        aCtr ++;
      } else {
        returned[totalCtr] = b[bCtr];
        bCtr++;
      }
      totalCtr++;
    }
    if (bCtr == b.length) {
      for (int x = aCtr; x < a.length; x ++) {
        returned[totalCtr] = a[x];
        totalCtr++;
      }
    }
    if (aCtr == a.length) {
      for (int x = bCtr; x < b.length; x ++) {
        returned[totalCtr] = b[x];
        totalCtr++;
      }
    }
    return returned;
